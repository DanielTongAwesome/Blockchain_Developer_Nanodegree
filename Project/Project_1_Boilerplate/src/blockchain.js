...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let blockHeight = await self.getChainHeight();
            blockObject.time = new Date().getTime().toString().slice(0,-3);

            // check whether its genesis block or not
            if (blockHeight > 0){
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if((time + (300000)) >= currentTime){
                ...
                reject('You can only submit star each 5 mins');
            }
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach(block => {
                ...
                if(block.height > 0) {
                    ...
                    if(blockHash != previousBlockHash){
                        errorLog.push(`Validation Error - Block Heigh: ${block.height} - Previous Hash don't match.`);
                    }
                }
                chainIndex++;
            });
            Promise.all(promises).then((results) => {
                ...
                results.forEach(valid => {
                    if(!valid){
                        errorLog.push(`Validation Error - Block Heigh: ${self.chain[chainIndex].height} - Has been Tampered.`);
                    }
                    chainIndex++;
                });
                resolve(errorLog);
